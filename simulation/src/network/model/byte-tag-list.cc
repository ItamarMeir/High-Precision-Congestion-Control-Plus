...
{
  NS_LOG_FUNCTION (tid << bufferSize << start << end);
...
{
  NS_LOG_FUNCTION (offsetStart << offsetEnd);
  if (m_data == 0)
    {
...
{
  NS_LOG_FUNCTION (adjustment << appendOffset);
...
{
  NS_LOG_FUNCTION (adjustment << prependOffset);
